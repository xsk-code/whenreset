...
Usage:
  node scripts/sync-tibo.mjs [--probe]              Read-only: report records awaiting confirmation
  node scripts/sync-tibo.mjs --add-tweet <text> --url <tweet-url> [--type regular|banked]
...
`);
  ...
  if (options.addTweet) {
    ...
    if (localIds.has(String(tweetId))) {
      ...
      console.log(`::probe::${JSON.stringify({ mode: "ingest", written: false, id: tweetId, reason: "duplicate" })}`);
      process.exit(0);
    }
    ...
    console.log(`::probe::${JSON.stringify({ mode: "ingest", written: !options.dryRun, id: tweetId, total: localResets.length + 1 })}`);
    process.exit(0);
  }
